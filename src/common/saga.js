...
const nonRestrictedEndpoints = [AUTH_SERVER_ENDPOINT, 'otp', 'reset-password', 'signup'];

let refreshTokenInProgress = false;

export function* handleAPIRequest(apiFn, ...requestData) {
    const { api, endpoint, testpoint, payload, onUploadProgress, onDownloadProgress, showErrorToast = true } = apiFn(...requestData);
    return yield call(invokeApi, api, endpoint, testpoint, payload, onUploadProgress, onDownloadProgress, showErrorToast);
}

function* invokeApi(api, endpoint, testpoint, payload, onUploadProgress, onDownloadProgress, showErrorToast) {
    ...
    const { response, error } = yield call(api, endpoint, testpoint, payload, onUploadProgress, onDownloadProgress);
