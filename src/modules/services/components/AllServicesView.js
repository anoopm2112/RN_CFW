...
const { SafeAreaView, FontelloIcon, Input, Card, Icon, FloatingAction } = components;
const { dimensionUtils: { convertWidth, convertHeight } } = utils;

const AllServicesView = (props) => {
    const {
        getService, allServices: { data, refreshing }, subscribeServices
    } = props;
    ...
    const renderItem = ({ item, index }) => {
        return (
            <Card key={item.id.toString()} shadow style={styles.cardStyle}>
                <TouchableOpacity onPress={() => RBSheetRef.current[item.id].open()} style={styles.viewStyle}>
                ...
                <RBSheet
                    ref={ref => {
                        if (!ref) return;
                        RBSheetRef.current[item.id] = ref;
                    }}
                    ...
                    <View style={{ padding: convertWidth(15) }}>
                        <View style={{ alignItems: 'flex-end', }}>
                            <TouchableOpacity onPress={() => { RBSheetRef.current[item.id].close() }} style={styles.closeBtnBSheet}>
                        ...
                        <View style={{ marginBottom: convertWidth(20), marginTop: convertHeight(20) }}>
                            <TouchableOpacity style={styles.buttonContainer} onPress={() => {
                                RBSheetRef.current[item.id].close();
                                subscribeServices({ remarks: remarks, serviceId: item.id })
                            }}>
                                <Text category="h5" style={styles.textStyle}>{I18n.t('subscribe')}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </RBSheet>
