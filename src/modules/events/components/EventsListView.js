...
const EventListView = (props) => {
    const { getUpcomingEvents, upcomingEvents: { data, refreshing }, userType: { assetIcons } } = props;
    ...
    const renderItem = ({ item }) => {
        return (
            <Card shadow style={styles.cardStyle}>
                <TouchableOpacity
                    onPress={() => props.navigateToEventDetails({ data: item, screen: 'upcomingEvents' })}
                    ...
                    {item.isRegistered && <View style={{ position: 'absolute', top: 0, right: 7 }}>
                        <Text category="h5" style={{ color: theme['color-warning-500'], fontWeight: 'bold' }}>Registered</Text>
                    </View>}
                </TouchableOpacity>
            </Card>
        )
